...
function isValidCard(number){
	do{
		...
		if(patt.test(answer) === false){
			alert('Ingrese solo números del 0 al 9');
		} else if(patt.test(answer) === true){
			...
			for(var j = 0; j < array1.length; j++){
				result += array1[j]; // Suma la variable result con cada número del array1.
			} 
			if(result % 10 == 0){ // Si la suma total dividida en 10 da módulo 0 que haga el siguiente paso.
	...
	} while (answer == "" || patt.test(answer) === false ); // Si no cumple la expresión regular patt o no ingresa nada que vuelva a preguntar.
